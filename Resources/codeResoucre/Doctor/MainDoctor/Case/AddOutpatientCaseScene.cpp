...
void AddOutpatientCaseScene::createCaseToNetWork(){
    ...
    char memberUrl[1000]={0};
    sprintf(memberUrl,"patientId=%s&doctorId=%s&type=%s&status=%s%s",userData["data"]["id"].GetString(),UserDefault::getInstance()->getStringForKey("id").c_str(),"new","outpatient",keyValue.c_str());
    char* url=memberUrl;
