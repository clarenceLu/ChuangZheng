...
void QRSprite:: draw(cocos2d::Renderer *renderer,const cocos2d::Mat4& transform,uint32_t flags)
{
...
bool QRSprite::initWithString(const std::string& code,int size)
{
    ...
    if (_qrcode != nullptr) {
        LayerColor::initWithColor(Color4B(255, 0, 0, 255), size, size);
        ...
        _texture->initWithData(_qrcode->data,code.length() , Texture2D::PixelFormat::A8, _qrcode->width, _qrcode->width, s);
